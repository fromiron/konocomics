...
- 등록된 배치의 Stop 훅은 결과 파일 유무와 과거 통지 여부에 관계없이 매 턴 종료 시 현재 단계 보고를 요구한다. 해당 턴의 동일 checkpoint를 이미 전송했다면 실제 성공 응답을 재사용한다. 전송 실패는 최종 응답에 명시한다. `stop_hook_active` 재진입에서는 반복하지 않으며 강제 종료·연결 단절 시 통지 보장은 없다. 부모 조정자는 실제 idle 상태도 확인한다.
...
- 배치 시작 시 notification_guard register-batch를 등록한다. 기존 한 작품 guard에 batch를 위장 입력하지 않는다. 등록된 모드의 실제 전송 응답으로 ack하고 통지 실패 때문에 수집·판정을 재실행하지 않는다. 부모가 이미 소비한 결과는 중복 발행하지 않는다.
