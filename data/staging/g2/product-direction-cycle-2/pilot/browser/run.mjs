...
const root = process.env.KONOCOMICS_G2_PILOT_ROOT;
if (root === undefined || root === "") {
  throw new Error("KONOCOMICS_G2_PILOT_ROOT is required");
}
...
const lowerComedySentence = "「ギャグ・コメディ」が控えめな点が、あなたの好みに合う作品です。";
const directionlessComedySentence = "「ギャグ・コメディ」があなたの好みに合う作品です。";
...
try {
  ...
  const explanationCopies = page.locator(".explanation-copy");
  const lowerComedySentenceCount = await explanationCopies.getByText(lowerComedySentence, { exact: true }).count();
  const directionlessComedySentenceCount = await explanationCopies.getByText(directionlessComedySentence, { exact: true }).count();
  assert(lowerComedySentenceCount === 3, "Expected exactly three lower-comedy explanations");
  assert(directionlessComedySentenceCount === 0, "Directionless comedy explanation remained visible");
  ...
  log = {
    ...
    interactions: {
      ...
      explanationRegression: {
        lowerComedySentence,
        lowerComedySentenceCount,
        directionlessComedySentence,
        directionlessComedySentenceCount,
      },
