...
- 공식 첫 화에서 전투 동세를 확인했으며, 인체·도구의 기능적 묘사와 스타일화된 표정이 함께 보여 작화 축을 중간대로 판정했다.
...
- 살해·추격·열세 전투가 빠르게 이어져 `pacing`, `darkness`, `motionImpact`가 높고, 개그·정서적 휴식은 상대적으로 적다.
- 공식 미리보기의 인체·갑옷·전장·해칭 정보량을 근거로 `artRealism`, `artDensity`를 높게, `visualSoftness`를 낮게 판정했다.
...
- 공식 첫 화의 연성 충격과 격투는 `motionImpact`를 지지하나 SD 개그와 단순화가 섞여 `artRealism`은 중간이다.
...
- 공식 미리보기는 현실적인 인체·공간과 세밀한 표정을 보여 주지만 핵심 표현은 정적인 심리 압박이어서 `artRealism`은 높고 `motionImpact`는 낮다.
...
- 공식 미리보기에서 전투가 존재하되 동세가 절제되어 `motionImpact=1`이며, 초반 공식 자료만으로 로맨스 서브플롯을 확정하기 어려워 `romance=unknown`이다.
...
- 거친 신체 훼손 표현과 각진 선을 근거로 `visualSoftness=0`이며, 극한 상황과 욕망의 불일치가 반복되어 `comedy=3`이다.
...
- 공식 본문의 극단적 원근·공 궤적·속도 강조를 근거로 `motionImpact=4`, 날카로운 선을 근거로 `visualSoftness=0`이다.
...
- 현실적 얼굴·나이·병원과 도시 공간을 근거로 `artRealism=4`이며, 동작보다 표정·구도·서스펜스를 강조해 `motionImpact=1`이다.
...
- 전투는 없지만 달리기·넘어짐·신체 개그의 동세가 반복되어 `motionImpact`는 `notApplicable`이 아니라 2다.
...
- 건축·배경 정보량과 전투 파괴 규모가 매우 크고 선은 거칠어 `artDensity`, `motionImpact`가 높고 `visualSoftness`는 낮다.
