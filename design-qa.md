## 2026-09-12 Taste 개선안 구현

- 사용자가 Grok 3명·Gemini 3명의 검토를 종합한 개선안의 확정·구현을 요청했다. 계획 `fa00012`, 미리보기 구현 `f21fdba`, 이후 반응형·포커스 수정은 이 기록과 함께 커밋한다. [확정 계획](docs/planning/redesign/taste-improvement-plan.md)과 `03` §4를 함께 갱신했다. 1200px 구현을 근거로 Taste 960px 계약을 약화했던 초안의 `AUTHORITY INVERSION`을 철회하고 전체 shell을 960px로 복원했다.
- 실제 개발 서버 `http://[::1]:3030`의 기존 Import/저장 경로로 준비된 기록을 사용했다. 사용자 `localhost:3030`의 프로필은 수정하지 않았다. UI에서 학원 제외를 저장한 뒤 미리보기 현재 선두 4개와 추천 Top 10의 선두 4개 href가 모두 일치했고, DNA 재진입에서 제외 유지와 새 비교 기준을 읽었다. 학원 meter의 `aria-valuenow`는 변경 전후 `1.379310344828`이었다. 확인용 변경은 모두 `自動`으로 원복했으며 마지막 새로고침 뒤 39개 보정 그룹의 선택이 모두 `自動`, 열린 범주 0개, 현재 미리보기 한 목록인 것을 재조회했다.
- 같은 프로필의 390×844 전후에서 상위 취향을 첫 줄 순위·이름·16px 강도와 다음 줄 근거로 압축했다. 최종 radar 라벨의 최대 bottom은 772.93px, 하단 탭 top은 779.33px였다. 이 프로필에서는 모든 라벨이 첫 화면에서 보이지만 일반 합격 조건으로 확대하지 않는다. 320px에서는 세로 스크롤로 하단 내용에 접근한다. 768px에서 재현한 28px 강도 겹침은 요약/레이더의 2열 시작을 1024px로 옮겨 해결했다. 상위 카드 3열 시작은 768px 그대로이며 768px 카드 폭 226.89px·1641px 카드 폭 157px 안에 가장 긴 강도 137.76px가 들어간다. 1641×1149에서 Taste shell은 960px다.
- 390px 근거 카드는 144px이며 두 작품 뒤 세 번째 작품 일부가 보인다. 기존 화살표 키로 다섯 번째 `はたらく細胞`까지 이동했다. 최종 track rect 12–362.67px, 마지막 링크 rect 214.67–358.67px로 4px focus ring까지 보인다. 표지·제목·근거·링크와 동일 SVG 데이터/좌표는 유지한다. 최근 기록의 제목은 desktop 20px/mobile 16px이며 마지막 기록도 하단 탭 위에서 읽을 수 있었다.
- 실제 Enter disclosure에서 BODY로 빠지던 포커스는 `group` key remount를 제거하고 URL을 직접 읽어 해결했다. 열기/닫기·뒤로/앞으로에 같은 버튼 포커스가 유지된다. 320px 선택 「除外」가 화면 밖에 남는 문제는 숨긴 1px radio 대신 공통 ChoiceChip 표시 label 전체를 네이티브 스크롤 대상으로 삼아 해결했다. 그룹·선반에 4px outline 여백을 확보했으며 저장 알림과 실제로 겹칠 때만 현재 포커스 항목을 스크롤한다. 네 범주의 6/22/7/4개 그룹을 키보드로 마지막 항목까지 접근했다. 공통 체크박스는 추천 정책 UI에서 선택 변경 없이 포커스 이동을 확인했다.
- 기존 `html min-width:320px`가 stable gutter와 합쳐져 320px 문서를 가로로 넘치게 했다. 최소폭 한 줄을 제거했으며 stable gutter는 유지한다. Taste·추천·Library의 320px document client/scroll width가 모두 305px였다. 현재 개발 탭의 error/warn 기록은 없었다. 임시 viewport는 기본 1280×720으로 복원했다.
- 변경 TS/TSX 6개 파일의 lint와 diff 공백 검사를 통과했다. 기존 Taste 회귀 파일에 상태 분기·URL 변경 포커스·알림 가림 검사를 보완했지만 **사용자 지시대로 unit/E2E·typecheck·build는 실행하지 않았다**. 실제 200% 브라우저 확대 단축키는 적용되지 않아 820×575의 재배치와 키보드 접근만 별도로 확인했다. 실제 확대·물리 터치·비영 safe-area·스크린리더·저장 실패·빈/계산 불가/누락 작품·normal/reveal/reduced-motion은 이번 실제 실행의 PASS가 아니다. 배포·푸시·PR은 수행하지 않았다.

증거는 `.tmp/taste-implementation-20260912/`에 있다. 전후 시각 비교는 `01-before-mobile.png`/`15-mobile-final-390.png`, `03-before-desktop.png`/`14-desktop-final-1641.png`; 실제 수정 원인은 `02`/`06`, `08`/`09`, `10`/`11`, `12`/`13`, 선반 최종은 `19-anchor-final-focus-390.png`에 기록했다. `preview-change-readback.json`, `recommendation-readback.json`, `responsive-readback.json`, `restored-adjustments-readback.json`은 화면·저장 경로의 관찰 결과다. 이미지의 가림·배치를 영속성이나 스크린리더 증거로 사용하지 않는다.

`03` §4 수용 기준 원문과 이번 확인 범위:

| 수용 기준                                                                                                                                                                                                                                                                     | 결과                                                                                                                                                                                 |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| reveal이 온보딩 완료 직후 1회만 재생된다(뒤로가기·새로고침 시 재생 안 됨).                                                                                                                                                                                                    | 미실행: 기존 reveal 판정·세션 마커를 변경하지 않았다.                                                                                                                                |
| reveal 판정 직후 `?reveal`이 즉시 제거되지만 anchors·요약·FactorBar A 시퀀스는 local decision으로 계속된다.                                                                                                                                                                   | 미실행: 해당 제어 흐름을 변경하지 않았다.                                                                                                                                            |
| 1200ms 전 뷰포트에 들어온 FactorBar는 전역 gate 뒤 시작하고, gate가 지난 뒤 처음 진입한 화면 밖 FactorBar는 추가 1200ms 지연 없이 섹션 내 60ms stagger만 적용해 각 1회 재생된다.                                                                                              | 미실행: 일반 진입 확인만 수행했다.                                                                                                                                                   |
| 모든 막대 값이 엔진 프로필 출력과 일치한다(스냅샷 테스트).                                                                                                                                                                                                                    | 스냅샷 미실행. 엔진·프로필 계산은 변경하지 않았으며 학원 한 막대의 전후 불변을 실제 DOM으로 확인했다.                                                                                |
| 초기 화면에는 5개 범주 summary가 모두 보이고 상세는 접혀 있다. disclosure를 열면 해당 범주의 상세만 노출되며 다른 범주의 상세는 닫힌다.                                                                                                                                       | 확인: 새로고침의 5개 접힘, 각 범주 단독 열림, URL 왕복과 버튼 포커스 유지.                                                                                                           |
| 장르 상세은 desktop 2열/mobile 1열 meter grid이며 radiogroup이 없다. 다른 네 범주의 5단 보정 control에는 영향이 없다.                                                                                                                                                         | 확인: 장르 10개 meter·radio 0개, 768px 344.33px씩 2열, mobile 1열. 네 보정 범주의 모든 39개 그룹 유지.                                                                               |
| 보정 radiogroup은 반복 segmented container나 선택 pill 없이 marker + label로 표시되고, 선택값은 filled marker와 text weight로도 구분된다. `除外` warning은 선택 시에만 표시된다.                                                                                              | 확인: 自動/控えめに/除外 선택·원복, 320px의 선택 label 및 outline 전체 노출.                                                                                                         |
| 열린 보정 범주는 desktop에서 `分析した好み` / `おすすめへの反映` 열 제목과 divider, mobile에서 행별 `おすすめへの反映` label을 제공한다. workspace heading과 설명은 분석값과 추천 설정을 별개로 설명한다.                                                                     | 확인: 768px 열과 divider·390/320px 행별 label, 기존 설명 끝에 결과 비교 위치를 추가.                                                                                                 |
| radio 변경 전후 해당 FactorBar의 `aria-valuenow`와 시각 길이는 동일하며, 막대 highlight 대신 구체적인 저장 `aria-live` message와 선두 최대 4작품의 recommendation preview가 해당 표시 범위의 영향을 전달한다.                                                                 | 확인: 학원 meter 값·길이 유지, 구체적 저장 알림과 변경된 선두 4작품. 실제 스크린리더 낭독은 미실행.                                                                                  |
| 열린 범주의 모든 기존 5단 보정 control을 keyboard로 접근할 수 있고 접고 다시 열어도 값이 유지된다.                                                                                                                                                                            | 확인: 네 범주 6/22/7/4개 그룹의 마지막 항목까지 Tab 접근, radio 방향키 이동, 테마 접힘/재열기·새로고침 저장값 복원.                                                                  |
| 보정 칩 변경 → Dexie 반영 → /recommendations 재진입 시 추천이 변한다.                                                                                                                                                                                                         | 확인: 학원 제외 저장→추천 Top 10의 선두 4개 변경·미리보기 일치→DNA 재진입 제외 유지. 이후 모두 自動으로 원복하고 새로고침 readback.                                                  |
| 미확인 축이 0값 축과 시각·접근성 DOM 시맨틱 모두에서 구분된다. 실제 스크린리더 낭독 검증은 제품 완료 후 선택적 접근성 감사 범위다.                                                                                                                                            | 해당 상태 미실행: FactorBar와 unknown 계산·의미를 변경하지 않았다.                                                                                                                   |
| 상위 취향 3개 각각에 근거 제목이 표시된다.                                                                                                                                                                                                                                    | 확인: 동일 실제 프로필의 320/390/768/1641px에서 세 항목의 근거 제목·순위·정성 강도 유지.                                                                                             |
| 일반 진입의 `normal` coaching banner는 근거 Shelf 뒤에서 `/onboarding` CTA와 DOM 문구만 사용하고, 최근 feedback의 동일 목적 링크를 중복하지 않는다. `high`와 최초 reveal에서는 숨긴다. 도달 불가능한 `low` UI fixture는 추가하지 않는다.                                      | 부분 확인: 실제 high에서 coaching banner 없이 기존 추가 링크 유지. normal/reveal은 미실행. low fixture를 추가하지 않았다.                                                            |
| 보정 preview의 before/after work ID가 같은 최신 기록·정책에 각 보정 설정을 적용해 결정론적으로 계산되고 영속 추천 결과를 URL이나 Router context에 저장하지 않는다. 동일/변경/양쪽 빈 목록/한쪽 빈 목록/계산 불가를 구분하며 사용자 표시는 제목·표지·선두 최대 4작품의 상태다. | 부분 확인: 동일 한 목록·변경 두 목록·재진입 기준 재설정과 추천 선두 일치. 빈/계산 불가/누락 분기는 코드·기존 회귀 파일을 보완했고 실행하지 않았다. ID의 화면/접근성 출력은 제거했다. |

## 2026-09-11 Library 관리 흐름·페이지네이션·전역 스크롤바 여백

- 기준 HEAD `9943442` 위 로컬 변경이다. 현재 사용자 `/library`의 17개 기록은 변경하지 않았다. 기존 중복 Shelf/상태 매트릭스를 단일 grid/list로 교체하고 검색→탭→조건·정렬·보기 순서, 0건 안내, 변경이 있을 때만 저장, 선택 진행 입력과 표지 재사용을 적용했다. UI·URL·화면 계약과 기존 회귀 파일을 함께 수정했다. Library 판매순 배너 제거는 답변이 없어 기존 승인 범위를 유지했다.
- 후속 사용자 결정으로 페이지당 최대 24개, 전체 기록에서 검색·필터·정렬 후 분할, `page` URL 복원·범위 정규화, 페이지에 한정한 cover resolver 대상을 적용했다. 별도 `http://[::1]:3030` 개발 origin에서 기존 Add/편집/Import UI로 0개→1개 저장→10개 모두 읽음→200개 혼합(198 Catalog·1 missing·1 canonical external)을 확인했다. 데이터 직접 주입·새 harness·provider mock은 사용하지 않았다. 입력 JSON은 규모·분기 확인용이며 실제 사용자 프로필이라는 뜻이 아니다.
...
| 수용 기준                                                                                                                                                                                                                                                                                                                          | 이번 확인                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 브라우저 재시작 후 모든 기록이 유지된다.                                                                                                                                                                                                                                                                                           | 부분 확인: 별도 개발 origin에서 UI 추가·저장 후 새로고침/새 탭 재진입으로 기록을 읽었다. 브라우저 프로세스 재시작은 수행하지 않았다.                                                                                                                                                                                       |
| 読んだ/途中でやめた/非表示 작품이 추천 후보에서 제외된다.                                                                                                                                                                                                                                                                          | 이번 재검사 생략: 추천 후보 selector·도메인 산식 변경 없음.                                                                                                                                                                                                                                                                |
| external entry가 Export에 포함되고 Import로 복원된다.                                                                                                                                                                                                                                                                              | 부분 확인: 기존 Import의 파일 검증→확인→교체→Library 경로로 canonical external을 복원했다. 이번 Export→Import 전체 왕복은 수행하지 않았다.                                                                                                                                                                                 |
| 하차 이유 편집이 다음 추천 감점에 반영된다.                                                                                                                                                                                                                                                                                        | 이번 재검사 생략: 하차 이유 직렬화와 감점 산식은 유지했다. 신규 무변경 저장 차단의 회귀 코드는 작성했으나 실행하지 않았다.                                                                                                                                                                                                 |
| Catalog 행 링크는 기존 `/works/{catalogWorkId}`를 유지하고 external 행만 canonical fixed-query URL을 사용한다.                                                                                                                                                                                                                     | 소스 확인: Catalog param 링크와 external fixed-query Link를 유지했다. 실제 external 편집창에서 상세 링크를 확인했다. 상세 목적지 진입은 이번 재검사 생략.                                                                                                                                                                  |
| 상태 count는 등록 기록, 결과 count/범위는 필터 결과와 현재 페이지에 일치한다. 전체 기록을 필터·정렬한 뒤 최대 24개를 표시하며 grid/list의 ID·순서·페이지가 같다. 페이지 이동/새로고침/범위 초과를 복원·정규화하고 현재 페이지 밖 표지를 일괄 요청하지 않는다. `updatedAt`/reaction/progress 외의 memo·시간 통계를 합성하지 않는다. | 확인: 사용자 17개를 한 번씩 표시. 별도 UI Import 10개 모두 읽음과 200개 혼합 기록 확인. 200개는 첫 24개/마지막 8개이며 전체 검색 NARUTO 1개, grid/list 2페이지 24개 ID 순서 일치, 새로고침 page=2 복원, page=999→9 정규화. 관찰한 API 12개는 모두 현재 24개 페이지에 속했고 다른 페이지 요청 0개였다.                      |
| 모바일 검색·wrap 탭·정렬·보기와 마지막 카드가 조작 가능하고 겹치지 않는다.                                                                                                                                                                                                                                                         | 확인 범위: 390×844에서 검색 44px·6개 탭 각각 44px·2열 171px·가로 넘침 0. 기존 17개 마지막 카드와 다음 Settings 링크를 하단 탭 위로 완전히 스크롤할 수 있었다. 200개 페이지 컨트롤을 포함한 물리 터치·실제 모바일 키보드는 미검증.                                                                                          |
| 검색 0건·빈 상태/favorite 조건을 구분하고 해제 시 관련 URL 조건만 변경한다.                                                                                                                                                                                                                                                        | 확인: 검색 0건/상태 조건 0건/즐겨찾기 AND, q만 해제와 state/favorite만 해제, 페이지 검색/상태 초기화를 실제 UI·URL로 확인했다.                                                                                                                                                                                             |
| 무변경 저장과 수정 후 원복 저장을 막고, 접힌 진행 값을 보존한다. 신규 기록의 명시적 확인·저장은 유지한다.                                                                                                                                                                                                                          | 확인: 기존 값에서 저장 비활성, 상태 수정 후 원복하면 다시 비활성. 개발 NARUTO의 진행 입력을 접은 뒤 저장하고 새로고침/재열기에서 reading·favorite·3권/24화 유지. 신규 판매순 확인 저장은 isNewRecord 경로 보존·회귀 작성, 이번 실제 신규 판매순 저장은 미검증.                                                             |
| 편집 제목/추가 검색으로 초기 포커스가 이동하고 Escape·포커스 복귀가 동작한다. 추가 검색 결과와 편집창에 같은 작품의 표지가 표시된다.                                                                                                                                                                                               | 확인: 편집 제목/추가 입력 초기 포커스, Arrow 키의 무의도 상태 변경 방지, Escape·원래 카드 복귀. external reading→completed 저장 뒤 카드가 필터에서 사라지면 reading 탭으로 복귀. 기존 카드/편집의 동일 표지 URL과 미등록 NARUTO 추가 검색의 실제 표지 로드 확인. 강제 200px 실패 fallback·느린 네트워크는 재현하지 않았다. |
| populated data-portability banner는 `/settings?section=data`만 열고 overall-empty image-half·검색 중·필터 결과 0건과 동시에 보이지 않는다.                                                                                                                                                                                         | 확인: overall-empty와 검색 0건에 데이터 배너 없음. 검색 해제 뒤 배너의 실제 Settings 데이터 링크로 이동. 후속 요청으로 desktop/mobile 전체 폭이며 1361px desktop의 목록·배너 폭이 모두 1152px였다.                                                                                                                         |
