...
1. 취향 적합도와 시장 인기도를 분리한다. 추천 산식에서 시장 신호는 tie-break로만 쓴다. 2026-09-11 사용자 승인 판매순 발견 배너는 별도 시장 탐색 표면이며 추천 점수·순위·설명에 합치지 않는다(`03` §5).
...
13. 원시 오프라인 모델 출력은 격리된 candidate일 뿐 Catalog 사실·판정의 권한이 아니다. 사용자가 승인한 `authorizedEvidencePanel`은 동결된 근거 패킷에서 claim을 다시 판정하고 `09`와 `authorized-evidence-panel-v1` 계약을 모두 통과한 별도 adjudication artifact에만 권한이 있다. 동일 source와 동일 accepted authority 입력은 모델 실행 이력과 무관하게 동일 결과를 낸다.
