...
- mobile: 히어로 표지 높이 최대 40vh, 이하 세로 스크롤. 「楽天ブックスで見る」와 검색 대체 버튼은 구매 영역의 전체 폭을 사용한다.
