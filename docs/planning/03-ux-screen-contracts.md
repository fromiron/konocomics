...
1. konomi 정적 2톤 로고(Slice 10) → 로고 reveal(Slice 11 시그니처 모먼트 #1, `04` §5.1)
...
4. 하단: 불호 요약(설정된 경우) + 「作品を追加して精度を上げる」 링크(§2의 `/onboarding` 추가 모드)
...
카드 제거: opacity+height 축소 240ms → 아래 카드 layout 이동(spring). 백필 카드: 하단에서 fade-up 200ms. C의 실제 목록 owner만 local `domMax`를 사용하고 페이지·사이드 패널·다이얼로그까지 감싸지 않는다. reduced-motion에서는 카드의 `layout`을 `false`로 두고 제거·백필을 즉시 반영한다. hover(desktop): 표지만 translateY(−2px), 120ms이며 그림자 상태는 보간하지 않고 즉시 바뀐다. 추천 화면에는 B 페이지 진입 모션을 적용하지 않는다.
...
3. 행 탭 → 상세 시트: readingState / reaction / 진행 권수 / 이유 편집. Catalog는 `/works/{catalogWorkId}`, external은 `/works/external?workId={encodedExternalWorkId}` 링크를 사용한다.
...
3. **このアプリ**: 버전, 데이터가 브라우저에만 저장됨 안내, `Supported by Rakuten Developers`와 Affiliate ID가 설정된 경우의 관계를 별도 항목으로 표시, 앱 자체 라이선스는 「未設定」, 「紹介をもう一度見る」(/?landing=1 — write-free 리다이렉트 우회). `package.json`의 `private: true`를 라이선스로 해석하지 않는다.
