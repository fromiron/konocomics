...
> `konocomics-project-plan.md` v0.2 대비 변경분(delta)을 한눈에 보기 위한 문서.
> 상세 근거는 `00-plan-audit.md`, 최종 사양은 02~07 문서 참조.
...
| P2 | 12개 제품 원칙 | §4 원칙 목록 | **KEEP** | 그대로 | 내부 모순 없음 |
...
| # | 영역 | 기존 제안 | Verdict | 최종 결정 | 이유 |
|---|---|---|---|---|---|
| R1 | 17개 Axis + Theme centrality + 3상태(known/unknown/notApplicable) | §13~16 | **KEEP** | 그대로. 단 Phase 1 데이터로 상관 진단 후 병합 여부 재평가 | 세부 취향의 가치가 검증 대상 가설 그 자체 |
| R2 | 고정 그룹 비중 (15/25/25/20/15) | §14 | **KEEP** | 그대로 | 상관 팩터 중복 가점 방지 |
| R3 | Best Anchor + Consensus Bonus ≤ +0.05 | §22 | **KEEP** | 그대로 | 소수 취향 보존의 올바른 해법 |
| R4 | Coverage 중립 수축, 가중치 재분배 금지 | §20 | **KEEP** | 그대로 | 결측 처리의 정석 |
| R5 | 부정 신호 3단 분리 | §23 | **KEEP** | 어휘·매핑을 확정(R6)한 뒤 그대로 | 구조는 옳음 |
...
| R9 | 시장 신호 tie-break, Bayesian 보정, maturity | §27 | **KEEP** | 그대로 | 원칙 1·2와 일치 |
| R10 | 추천 리스트 제약 (MMR 미사용) | §28 | **KEEP** | 그대로 | 작은 후보 풀에 적정 |
| R11 | 설명 Cluster + 그룹당 1개 | §29 | **KEEP** | 그대로 | 동어반복 방지 |
...
| A11 | Catalog 빌드 파이프라인 (CSV→검증→JSON) | §38~39 | **KEEP** | 그대로 (스크립트 구성 유지) | 건전 |
| A12 | 폴더 구조 | §38 | **REFINE** | `infrastructure/analytics` 제거, `lib/strings`(일본어 문자열) 추가 등 소폭 정리 (`05` §5) | A10·P9 반영 |
...
| V13 | 애니메이션 예산 (지속 1개/화면 등) | §42 | **KEEP** | 모션 분류 체계로 정식화 (`04` §6) | 건전한 원칙 |

## 검증·순서

| # | 영역 | 기존 제안 | Verdict | 최종 결정 | 이유 |
|---|---|---|---|---|---|
| S1 | Phase 0~6 (7단계) | §40 | **MERGE** | 5단계로 재편: 사전+엔진 → 50작품 sanity → 150작품 블라인드(GO 게이트) → Web MVP → 폴리시+PWA | R12 반영, 블라인드 하니스 정의 추가 (감사 S-1) |
| S2 | 블라인드 테스트 방법 | 지표만 정의 | **REFINE** | 엔진·Catalog 재사용하는 로컬 웹 하니스로 진행, 결과 JSON 기록 | 실행 방법 미정의 해소 |
| S3 | GO/REVISE 기준·핵심 지표 | §31 | **KEEP** | 그대로 | 방향성 판단 기준으로 적정 |
| S4 | E2E 5개 시나리오 | §41 | **KEEP** | 그대로 (`07` §4) | 과하지 않은 적정 범위 |
