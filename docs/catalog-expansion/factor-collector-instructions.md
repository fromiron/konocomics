...
사용자 최신 결정에 따라 신규 일반 작품은 **루나1~5 고정 작업방의 Luna max 수집 → Astra low 조정자의 저장·백업·동결 → 같은 Luna 작업방 판정**으로 처리한다. 모델은 `gpt-5.6-luna`, 추론은 `max`다. 이전 3작품 시험은 이력으로 보존하며 재시험·완료 결과의 전수 재판정은 하지 않는다.

수집 완료 시 부모에 `COLLECTION_READY`와 workId·경로·SHA를 전송하고 성공을 확인한 뒤 같은 작품의 동결 입력을 기다린다. 그동안 해당 방에 다른 작품을 배정하지 않는다. 원문 보존·동결 전 수치 판정 금지·공유 writer 금지 규칙은 동일하다. 세부 절차는 `01a-promotion-method-operational-amendment.md`의 Luna 수집·판정 연속 운영을 따른다.

정상 수집에서 모델의 책임은 자료 선택·실제 독해·출처별 관찰·불확실성이다. 경로·시각·receipt·JSON 직렬화는 아래 기존 helper에 맡긴다. 같은 관찰을 인계 문서·별도 준비 보고서·반복 상태 메시지로 다시 작성하지 않는다. 종료 시 research 경로·SHA·구체적 남은 gap/재개 조건만 반환한다. 이미 충분한 일반 근거를 모았다면 선택적 Art나 전권 미독해 때문에 연장하지 않는다. 비는 작업 용량만큼만 새 작품을 배정한다.

수집자는 공개된 동일 만화 자료를 읽고 원문·출처별 관찰을 남긴다. 판정·승격은 별도 동결 뒤 Luna max의 권한이다. 현재 배정은 고정 Luna max 5개 작업방이며 방당 한 작품을 맡는다. 조정자 포함 17개는 상한이며 인원을 채우기 위한 신규 위임은 하지 않는다. 일반 작업은 이 파일과 배정 brief/researchRefs를 읽는다. 이전 실험 대화·전체 STATE·planner·완료한 판정 보고서를 새 작품 맥락으로 넘기거나 반복해서 읽지 않는다.

조정자는 고정 Luna 작업방에 작품을 배정할 때 현재 작품의 ID·정체/ISBN·gap·기존 research/HOLD 경로·담당 출력 경로만 전달한다. 연결된 미소비 관찰·반대 근거·최신 HOLD는 생략하지 않으며 필요하면 원본 경로로 읽는다. 동일 자료의 국소 보정은 원래 담당자가 계속할 수 있다. 의미 있는 맥락을 자르는 토큰 상한이나 출처 제한은 만들지 않는다.
...
## 사용자 사전 수집 자료 우선 — 2026-09-19

사용자 승인에 따라 `.workspace/user-sources/<workId>.json`이 있으면 신규 출처 탐색 대신 해당 URL 검증을 기본으로 한다. README는 리드 작성 규격이며 JSON의 요약·서지 값 자체가 판정 근거나 현재 Catalog 정체성을 대체하지 않는다.
...
- 추가 검색은 제공 URL 접근 실패, 다른 작품/판본, 실제 필수 근거 누락을 확인한 경우에만 한다. gap을 명시한 좁은 검색 1회(최대 2개 쿼리)와 결과 직접 열람으로 제한한다. 해결되지 않으면 확보한 research와 gap/재개 조건을 부모에게 보내고 종료한다. 반복 검색·다른 검색어로 연속 확장하지 않는다. 더 조사할지는 조정자가 새 근거 가능성을 보고 별도 배정한다.
