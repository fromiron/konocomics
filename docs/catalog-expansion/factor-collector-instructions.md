...
후속 사용자 승인으로 신규 작품의 **고정 Sol medium 채팅 수집 → 조정자 저장·동결 → 같은 Sol 채팅 판정** 시험을 진행한다. 시험 배정은 아래 Luna 기본 배정보다 우선하며 원문 보존·동결 전 수치 판정 금지·공유 writer 금지 규칙은 동일하다. 수집 완료 시 부모에 `COLLECTION_READY`와 경로·SHA를 전송하고 성공을 확인한 뒤 동결 입력 전달을 기다린다. 이미 배정된 Luna 수집은 보존·소비한다. 세부 절차는 `01a-promotion-method-operational-amendment.md`의 Sol 수집·판정 연속 시험을 따른다.
...
수집자는 공개된 동일 만화 자료를 읽고 원문·출처별 관찰을 남긴다. 판정·승격은 별도 동결 뒤 Sol medium의 권한이다. 현재 배정은 Luna xhigh, 2026-09-15 사용자 지시로 조정자 포함 최대 17개(서브에이전트 16개)이며 수집자가 다시 위임하지 않는다. 일반 작업은 이 파일과 배정 brief/researchRefs를 읽는다. 이전 실험 대화·전체 STATE·planner·완료한 판정 보고서를 새 작품 맥락으로 넘기거나 반복해서 읽지 않는다.
