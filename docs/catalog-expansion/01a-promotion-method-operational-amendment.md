...
- 실제 판정값의 사전 앵커만 요구한다. 낮은 값에 높은 값의 반복성·강도·장기성·성공을 요구하지 않으며 구체적인 소개·텍스트 리뷰로 판단할 수 있으면 추가 출처 수, 직접 본문, 전권 독해를 의무화하지 않는다. 작품 전체 기준 변경만으로 기존 승인 팩터의 신뢰성이나 자격을 낮추지 않는다.
