...
3. `docs/planning/08-tanstack-start-migration.md` — framework·Router·URL·SSR·server migration 계약
4. `docs/planning/05-architecture.md` — 스택·경계·영속성·API 계약·빌드 파이프라인
5. `docs/planning/03-ux-screen-contracts.md` — 화면별 구현 계약·수용 기준
6. `docs/planning/04-visual-interaction-spec.md` — dark token·모션·시그니처 모먼트
7. `docs/planning/redesign/` — 확정 7화면의 구도·밀도·페이지별 구현 번들
8. `docs/planning/06-implementation-plan.md` — 슬라이스 순서·완료 기준
9. `docs/planning/07-acceptance-test-plan.md` — 테스트 계약
...
- **서버 경계:** 서버 코드는 TanStack Start server route의 `/api/rakuten/search`·`/api/rakuten/item` 둘뿐. 임의 server function·새 server route·database·auth·runtime LLM 추가 금지. `08` §3의 route별 SSR/client 경계를 따른다.
...
pnpm catalog:build          # data/source CSV → src/data/generated/catalog-v1.json
...
- Catalog 상세은 150개 bundled ID prerender로 닫고, external은 고정 정적 셸 `/works/external?workId=<ExternalWorkId>`에서만 client lookup한다. external v1 ID/key는 immutable이며 Export/Import 호환을 유지한다.
- 슬라이스 11까지의 Next.js 제품이 migration baseline이다. 다음 구현 작업은 `08`의 M0~M10 TanStack Start migration과 dark-only 7화면 redesign이며, 추천/Dexie/Rakuten 계약은 동결한다.
